...
// WorkerPool runs N concurrent Parquet conversion workers.
...
func (p *WorkerPool) runWorker(ctx context.Context, id int) {
	log := p.log.With("worker", "parquet", "id", id)
	poll := minPoll
	...
	for {
		...
		job, err := p.queue.Claim(ctx, domain.StageParquetConversion)
		if err != nil {
		...
		if err := p.service.Process(ctx, job); err != nil {
			if errors.Is(err, domain.ErrPolicyBlocked) {
				...
				cancel()
				log.Info("parquet conversion skipped by policy", "file_id", job.FileID)
				continue
			}
			log.Warn("parquet conversion failed", "file_id", job.FileID, "err", err)
			finalizeCtx, cancel := finalizeContext(ctx)
			...
			finalizeCtx, cancel := finalizeContext(ctx)
			if err := p.queue.Ack(finalizeCtx, job.ID, job.FileID, domain.StageParquetConversion); err != nil {
